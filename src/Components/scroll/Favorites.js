      ...
      {favorites.length > 0 ? 
        favorites.map((book) => (
          <div key={book.id} className='book'>
            ...
            <div>
              {favoritesChecker(book.id) ? (
                <button className='fbtn' onClick={() => removeFromFavorites(book.id)}>Remove from Favorites</button>
              ) : (
                  <button className='fbtn' onClick={() => addToFavorites(book)}>Add to Favorites</button>
                )}
            </div>
          </div>
        )):<h1>You don't have any Favorites yet!</h1>
